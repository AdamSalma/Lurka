...
export function shouldRequestBoard({ board, settings }) {
  ...
  console.log(
    `shouldRequestBoard(): ${lastRequested} > ${requestThrottle} = ${
      lastRequested > requestThrottle
    }`
  );
...
export function boardCachedAndRecent({ cache, settings }, boardID) {
  ...
  if (!board) {
    ...
    console.warn(
      `DID EXIST. ${secondsAgo(board.receivedAt)} < ${maxBoardAge} = ${
        secondsAgo(board.receivedAt) < maxBoardAge
      }`
    );
    console.warn(board);
  }
