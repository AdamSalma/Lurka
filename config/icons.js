module.exports = {
  // navbarMenu: 'navicon',
  navbarForwards: "arrow-big-right",
  navbarBackwards: "arrow-big-left",

  navbarInfo: "information",
  navbarRefresh: "reload",
  navbarNewThread: "mode_edit",
  navbarArchive: "folder",

  navbarAccount: "person-add",
  navbarBookmark: "bookmark-o",
  navbarDB: "data",
  navbarEye: "schedule",
  navbarSettings: "settings",
  navbarTheme: "paint-brush-2",
  navbarMenu: "menu",
  navbarToTop: "arrow-doubled-up",
  navbarSearch: "search2",
  navbarDownloads: "arrow-down2",
  navbarSort: "sort-alphabetically",
  navbarFilter: "filter",
  navbarChevron: "chevron-down",

  // subNavbarClose: 'chevron-up',
  //
  plus: "plus",

  searchMagnify: "search2",
  searchMagnify4: "search4",
  searchClose: "times",

  videoFullscreen: "fullscreen",
  videoDownload: "arrow-down2",
  videoPlay: "play_arrow",
  videoPause: "pause",
  videoError: "warning-alt",
  videoVolumeOff: "volume_off",
  videoVolumeMute: "volume_mute",
  videoVolumeLow: "volume_down",
  videoVolumeMedium: "volume_up",
  videoVolumeHigh: "volume_up",

  boardPostReplyCount: "comment",
  boardPostImageCount: "image",
  boardToTopChevron: "chevron-up",

  // threadPostImageFullscreen: 'arrow-expand',

  threadPostMenu: "dot-3",
  threadPostControlsReport: "flag",
  threadPostControlsBookmark: "bookmark",
  threadPostControlsReply: "quote",
  threadPostControlsImageSearch: "search-2",
  threadPostControlsHide: "minus-circle",

  threadControlsCinema: "view_carousel",
  threadControlsClock: "eye-plus",

  threadMediaExpand: "zoom_in",

  threadContextMenuDownload: "download",
  threadContextMenuReverseSearch: "search2",
  threadContextMenuMagnify: "zoom_out_map",
  threadContextMenuImage: "image",
  threadContextMenuVideo: "zoom_in",
  threadContextMenuReply: "reply",

  watchPanelClose: "times",
  watchPanelUpdate: "reload",

  boardToolbarSort: "sort-alphabetically",
  boardToolbarFilter: "filter",
  boardToolbarLayout: "layout",
  boardToolbarInfo: "info",
  boardToolbarOpen: "chevron-up",
  boardToolbarClose: "chevron-down",

  chevronDown: "chevron-down",

  subheaderToolbarSort: "sort-alphabetically",
  subheaderToolbarFilter: "filter",
  subheaderToolbarChangeLayout: "layout",
  subheaderToolbarNewThread: "mode_edit",
  subheaderToolbarRefresh: "refresh-1",
  subheaderToolbarArchive: "folder",
  subheaderToolbarSettings: "settings",
  subheaderToolbarRedo: "forwards",
  subheaderToolbarUndo: "backwards",

  dashboardReturn: "arrow-doubled-left",

  cinemaPreviousMedia: "chevron-left",
  cinemaNextMedia: "chevron-right",
  cinemaClose: "times",

  postSubmit: "paper-plane",
  postClose: "times",

  fileInput: "attach_file"
};
